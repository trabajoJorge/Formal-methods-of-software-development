...
ensures append(take(i,xs),Cons(elem(xs,i),drop(i+1,xs))) == xs
{
	if i == 0 {}
		...
		assert append(take((i-1),xs.tail),Cons(elem(xs.tail,(i-1)),drop((i-1)+1,xs.tail))) == xs.tail;
		...
		calc {
			...
			Cons(xs.head, append(take((i-1),xs.tail),Cons(elem(xs.tail,(i-1)),drop((i-1)+1,xs.tail))));
			append(take(i,xs),Cons(elem(xs,i),drop(i+1,xs)));
		}
	}
	assert append(take(i,xs),Cons(elem(xs,i),drop(i+1,xs))) == xs;
}
...
{  // METODO PARA OBTENER EL MINIMO INDICE
	var k := j;
	im := j;
